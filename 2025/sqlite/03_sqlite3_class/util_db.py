...
class MyDB():
    ...
    # CRUD(Insert)
    def insert_record(self, name, comment):
        ...
        sql = """
            ...
            """.format(self.db_table)
        cur.execute(sql, (None, name, comment, self.get_time()))
        con.commit()# Important
        con.close()
    ...
    def read_records(self):
        ...
        cur = con.cursor()
        sql = """
            SELECT * FROM {0} ORDER BY uid DESC
            """.format(self.db_table)
        cur.execute(sql)
    ...
    # CRUD(Update)
    def update_record(self, uid, name, comment):
        print("update_record:", uid)
        # SQLite
        con = sqlite3.connect(self.db_path)
        cur = con.cursor()
        sql = """
            UPDATE {0} SET name=?, comment=?, time_stamp=? WHERE uid=?
            """.format(self.db_table)
        cur.execute(sql, (name, comment, self.get_time(), uid))
        con.commit()# Important
        con.close()
